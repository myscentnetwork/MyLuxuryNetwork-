...
export default function RetailLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/retail/auth", {
        method: "POST",
      ...
      document.cookie = `retail_auth=${data.retailer.id}; path=/; max-age=86400`;
    ...
    <div className="min-h-screen flex items-center justify-center bg-luxury-black p-4">
      <div className="bg-luxury-dark p-8 rounded-2xl shadow-2xl w-full max-w-md border border-luxury-gray">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2">Retail Portal</p>
        </div>
        ...
        <p className="text-center text-gray-500 text-xs mt-6">
          Luxury Marketplace Retail Portal
        </p>
      </div>
    </div>
  );
